...
### Просмотр ключей
На экране просмотра ключей вы можете фильтровать, добавлять, редактировать, удалять, перемещать, менять порядок и копировать нужный TOTP ключ.

- `1`…`9` - быстрый выбор и копирование N-го ключа в текущем виде (слот `0` = 10-й, как с вкладками/оружием в играх)
- `a` - добавить новый ключ (в пикер папок подставляется текущая папка)
- `e` - редактировать выделенный ключ (название/описание)
- `d` - вызвать удаление (Enter - подтвердить, Esc - вернуться назад)
- `m` - переместить выделенный ключ в другую папку
- `shift+↑` / `shift+↓` или `K` / `J` - поменять порядок ключа внутри текущей папки (порядок сохраняется в vault)
- `enter` - скопировать в буфер обмена. При копировании левая граница становится толще.
- `/` - фильтрация по имени

Перед каждой строкой — метка слота (`[1]`, `[2]`, …, `[0]` для 10-го).

### Папки
Ключи можно группировать по папкам (Работа / Дом / Личное / …). Список папок — это стартовый экран; сверху находится синтетическая строка **All keys**, которая показывает все ключи.

На экране папок:

- `0`…`9` - быстрый переход в папку по слоту N (слот `0` — всегда **All keys**)
- `a` - добавить новый ключ (подставляется выделенная папка — или Default на *All keys*)
- `n` - создать новую папку
- `e` - переименовать выделенную папку
- `d` - удалить выделенную папку (её ключи переносятся в **Default**; саму `Default` удалить нельзя)
- `Enter` - открыть папку
- `Esc` / `q` - выйти

Перед каждой строкой — метка слота (`[0]` для All keys, `[1]`, `[2]`, …).

### Добавление ключей
Чтобы добавить новый ключ, введите **Имя** и **SecretKey**, Описание по желанию.\
SecretKey формата base32 (пробелы и нижний регистр допускаются и автоматически нормализуются), иначе вернётся ошибка.\
Четвёртое поле — пикер папок: пока он в фокусе, `←` / `→` (или `h` / `l`) переключают папку. При открытии из папки или со списка папок — эта папка предвыбирается.
